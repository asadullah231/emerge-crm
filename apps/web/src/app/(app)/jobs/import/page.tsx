...
import { autoMapJobs, JOB_IMPORTABLE_FIELDS, parseCsv, type JobImportableField } from "@/lib/csv";
import { trpc } from "@/lib/trpc/client";

type Mapping = Record<JobImportableField, number | null>;

/** CSV import wizard for job openings (M17b, mirrors the candidates importer). */
export default function ImportJobsPage() {
  ...
  const importMut = trpc.jobs.importJobs.useMutation();
  ...
  const mappedRows = useMemo(() => {
    ...
    return rows.map((cols) => {
      ...
      for (const { key } of JOB_IMPORTABLE_FIELDS) {
  ...
  const onFile = async (file: File) => {
    ...
    try {
      ...
      setRows(parsed.rows);
      setMapping(autoMapJobs(parsed.headers));
    } catch {
  ...
  const run = (dryRun: boolean) =>
    importMut.mutate({ rows: mappedRows, createMissingCompanies: createMissing, dryRun });

  const requiredMapped = mapping && mapping.title !== null && mapping.companyName !== null;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold">Import job openings</h1>
        <p className="mt-1 text-sm text-[var(--muted)]">
          Upload a CSV, map the columns, preview, then import. Each row needs a job title and a
          client company.
        </p>
      </div>
      ...
      {mapping ? (
        <section className="space-y-3 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {JOB_IMPORTABLE_FIELDS.map((field) => (
              <div key={field.key}>
          ...
          {!requiredMapped ? (
            <p className="text-sm text-red-600">
              Job title and client company must be mapped to continue.
            </p>
          ) : null}
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            />
            Create client companies that do not exist yet
          </label>

          <div className="flex gap-2">
            <Button
              variant="outline"
              disabled={!requiredMapped || importMut.isPending}
              onClick={() => run(true)}
            >
              {importMut.isPending ? "Checking..." : "Preview"}
            </Button>
            <Button disabled={!requiredMapped || importMut.isPending} onClick={() => run(false)}>
  ...
  return (
    <div className="space-y-2 text-sm">
      <ul className="grid grid-cols-2 gap-x-6 gap-y-1 sm:grid-cols-4">
        ...
        <li>
          Valid: <strong>{report.valid}</strong>
        </li>
        <li>
          Jobs created: <strong>{report.created}</strong>
        </li>
        <li>
          New clients: <strong>{report.companiesCreated}</strong>
        </li>
      </ul>
