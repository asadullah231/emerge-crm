...
| Zoho feature                             | Priority     | Our equivalent                                                                                                                                                                                                                                          | Data    | UI                                  | Backend                                      | Migration        | Milestone               |
| ---------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------------------------------- | -------------------------------------------- | ---------------- | ----------------------- |
| Email send + templates + log to timeline | SHOULD       | email from a record (SMTP/Resend) with templates + {{merge}} fields, thread + audit log, mail merge to many, inbound reply threaded via Reply-To token                                                                                                  | yes     | composer + templates + mail-merge   | send worker + inbound webhook                | no               | done (M13)              |
| Two-way email sync                       | NICE         | full mailbox sync (beyond reply capture)                                                                                                                                                                                                                | yes     | thread view                         | sync worker                                  | no               | post-1.0                |
...
| Reports & dashboards (scoped)            | SHOULD       | 6 agency KPI reports (funnel+conversion, submissions/sourcer, time-in-stage, time-to-first-submission, client health, leaderboard) with date/owner/client filters, CSV export + scheduled email delivery. Full drag-drop report builder stays post-1.0. | done    | /reports + filters + CSV + schedule | @emerge/reports aggregates + delivery worker | reads live       | done (M14)              |
